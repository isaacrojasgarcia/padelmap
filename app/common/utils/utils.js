(function () {
    ...
    angular.module('olaf.common.utils', [])
        .factory('cookies', function () {
            return {
                ...
                check: function () {
                    this.create('ramsonCookiesEnabled', 'ramson');
                    if (this.read('ramsonCookiesEnabled') !== null) {
                        this.erase('ramsonCookiesEnabled');
                        return false;
                    }
                    return true;
                },
        ...
        .factory('slugify', function () {
            return function (str, options) {
                //return str.toLowerCase().replace(/[^\w]+/g, '-').replace(/ +/g, '-').replace(/_+/g, '-');
                var maxLength = (options && options.maxLength) || 50; // Max slug length
                var spaceChar = (options && options.spaceChar) || '-'; // Space replacement
                var invalidChar = (options && options.invalidChar) || '_'; // Invalid char replacement
